...
library (dplyr)
    ...
    #library (dtplyr)
    #df |>
    #  dtplyr::separate (symptoms, into = c("sym_1", "sym_2"), sep = ",", extra = "merge")
    install.packages ("tidyr")
    library (tidyr)
df |>
...
df0 <- data.frame (
  case_ID = c(1:6),
  symptoms = c("jaundice, fever, chills", #患者1
               "chills, aches, pains", #患者2
               "fever, NA", #患者3
               "vomiting, diarrhoea", #患者4
               "bleeding from gums, fever", #患者5
               "repid pulse, headache"), #患者6
...
df <- df1 |>
  mutate (is_educator = case_when (
    str_detect (occupations,
                regex ("teach|tutor|university", ignore_case = T))&
      str_detect (occupations,
                  regex ("admin", ignore_case = T),
                  negate = TRUE ~ "Educator",
                  TRUE ~ "Not an educator")
  ))
#No matter how i look at it, there two pieces of logic just don't flow.




#Identify the position of a pattern
str_locate ("I wish", "sh")
#            123456


phrases <- c("I with", "I hope", "he hopes", "He hopes")
str_locate (phrases, "h") #The position of the *first* occurrence of the pattern


str_locate_all (phrases, "h") #The positions of *all* occurrences of the pattern



#Extract the matched pattern (s)
#str_extract_all () returns the matched patterns themselves. 
#This is especially useful when searching for multiple patterns using "OR (|)".
#For wxample, suppose you want to find words that match any of the patterns 
#"Teach", "prof" or "tutor" within a list of occupation strings 
#(see the previous section).

str_extract_all (occupations, "teach|prof|tutor")

#In contrast, str_extract () returns only the first matched pattern.
#As a result, the output is a character vector containing at most one pattern for each input string.
#If no pattern matches, NA is returned.
#This NA can be removed by wrapping str_extract () with na.exclude ().
#In the example below, note that the second matched pattern in the third string is not shown.
...
#10.6
#10.7
#10.8 Regular Expressions and Special Characters
test <- "A-AA-AAA-AAAA"
...
pt_note <- "Patient arrived at Broward Hospital emergency ward at 18:00 on 6/12/2005. Patient presented with radiating abdominal pain from LR quadrant. Patient skin was pale, cool, and clammy. Patient temperature was 99.8 degrees farinheit. Patient pulse rate was 100 bpm and thready. Respiratory rate was 29 per minute." #copy and paste
