...
#ifelse (condition, processing when the condition is TRUE, processing when the condition is fALSE)
scires <- c(58, 100, 81, 97, 71, 61, 47, 60, 73, 85)
pf2 <- ifelse (score >= 60, "Pass", "Fail")
pf2


Noodle <- data.frame (
  id = 1:10,
  noodle = c(1, 1, 2, 1, 3, 1, 9, 2, 1, 9)
)
Noodle

#Suppose we want to assign NA to the ramen column when ramen = 9, and otherwise keep the original values unchanged. We then overwrite the ramen column in the Ramen data frame with the result. This can be done using the following codel:
Noodle$nodle <- ifelse (Noodle$noodle ==9, NA, Noodle$noodle)
Noodle
...
#If ifelse() is overused, the readability of the code may decrease. Therefore, when returning different values basd on multiple conditions,
#it is preferable to use the case_when () functon from the {dplyr} package.
...
my_calc <- function (x, y, operation) {
  switch (operation,
          ...
          "*" = x*y,
          "/" = x/y,
          stop ("Only +, -, *, and / can be used as values foroperation."))
}
...
#my_calc (5, 3, operation = "^")
#Error in my_calc(5, 3, operation = "^") : 
#  Only +, -, *, and / can be used as values foroperation.
#my_calc2 (5, 3, operation = "^")
#Error in match.arg(operation) : 'arg' should be one of “+”, “-”, “*”, “/”
