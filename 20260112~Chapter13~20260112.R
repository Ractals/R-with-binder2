...
#filter ()
#Slicing by group
linelist101 <- linelist00 |>
  group_by (hospital) |>
  arrange (hospital, date_hospitalisation) |>
  slice_head (n = 5) |>
  arrange (hospital) |>                            #For display purposes
  select (case_id, hospital, date_hospitalisation) |> #For display purposes
  mutate (date_hospitalisation = as.Date(date_hospitalisation, origin = "1899-12-30"))
linelist101
#~20:01

#Filtering by grop size
linelist102 <- linelist00 |>
  as_tibble () |>
  add_count (hospital) |> #Add the number of rows corresponding to admissions to the same hospital as this row
  select (hospital, n, everything ()) |>
  mutate (date_infection = as.Date(date_infection, origin = "1899-12-30"))
linelist102

linelist103 <- linelist102 |>
  add_count (hospital) |>
  filter (n < 500)
linelist103

#13.8 Transforming grouped data
linelist104 <- linelist00 |> 
  as_tibble () |>
  add_count (hospital) |>
  select (hospital, n, everything ()) |>
  mutate (date_infection = as.Date(date_infection, origin = "1899-12-30"))
...
linelist105 <- linelist104 |>
  add_count (hospital) |>
  filter (n < 500)
linelist105

#Grouped data by hospital (changes to the line list have not yet been saved)
#New columns
#Average number of days to admission by hospital (rounded to one decimal place)
#Difference betwee  each row's delay and the hospital's average delay (rounded to one decimal place)

#Select specific rows for display purposes

#13.9 Selecting grouped data
#select () works on grouped data, but grouping variables are always included, even if they are not explicitly specified in select ().
#if you do not want to include the grouping  ariables, use ungroup () first.
