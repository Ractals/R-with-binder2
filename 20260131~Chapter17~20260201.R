...
tab_age_outcome <- table (
  forcats::fct_na_value_to_level (linelist$age_cat, na_level = "Missing"), 
  forcats::fct_na_value_to_level (linelist$outcome, na_level = "Missing"))
tab_age_outcome <- stats::addmargins (
  tab_age_outcome,
  margin = c(1, 2),
  FUN = sum
)
df_age_outcoume <- as.data.frame.matrix (tab_age_outcome)
colname (df_age_outcome)[colnames (df_age_outcome) == ""] <- "Total"
df_age_outcome |>
  tibble::rownames_to_column (var = "Age Category")
...
tab_age_outcome <- table (
  forcats::fct_na_value_to_level (linelist$age_cat), 
  forcats::fct_na_value_to_level (linelist$outcome))
tab_age_outcome <- stats::addmargins (tab_age_outcome)
df_age_outcome <- as.data.frame.matrix (tab_age_outcome)

colnames (df_age_outcome)[is.na (colnames (df_age_outcome)) | colnames (df_age_outcome) == ""] <- "Total"
df_age_outcome <- tibble::rownames_to_column (df_age_outcome, var = "Age Category")
df_age_outcome
#Using AI doesn't mean errors just magically disappear.
#If i don't correct the course myself, i end up stuck in error hell forever. Guess that's how it is.
